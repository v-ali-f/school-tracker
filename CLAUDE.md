...
- Сервер: адрес и пользователь задаются через согласованный SSH-профиль; пароли в репозитории не хранятся
- Путь: `/home/user/portal`
- `.env` на сервере содержит `DATABASE_URL`; значение хранится только на сервере
...
- Пароль администратора на сервере хранится вне репозитория и не должен
  фиксироваться в deploy-скриптах или документации.
