...
class ControlWork(db.Model):
    ...
    WORK_KIND_CONTROL = "control"
    WORK_KIND_THEMATIC = "thematic"
    WORK_KIND_INTERMEDIATE = "intermediate"
    WORK_KIND_TEST = "test"
    WORK_KIND_SCHOOL_DIAGNOSTIC = "school_diagnostic"
    WORK_KIND_START = "start"
    WORK_KIND_DICTATION = "dictation"

    WORK_KIND_CHOICES = [
        (WORK_KIND_CONTROL, "Контрольная работа"),
        (WORK_KIND_THEMATIC, "Тематическая работа"),
        (WORK_KIND_INTERMEDIATE, "Промежуточная аттестация"),
        (WORK_KIND_TEST, "Тест"),
        (WORK_KIND_SCHOOL_DIAGNOSTIC, "Школьная диагностическая работа"),
        (WORK_KIND_START, "Стартовая работа"),
        (WORK_KIND_DICTATION, "Диктант"),
    ]
    WORK_KIND_LABELS = dict(WORK_KIND_CHOICES)
    ...
    retention_until = db.Column(db.Date, nullable=True)
    dictation_grade5_spelling_max = db.Column(db.Integer, nullable=False, default=0, server_default='0')
    dictation_grade5_punctuation_max = db.Column(db.Integer, nullable=False, default=0, server_default='0')
    dictation_grade4_spelling_max = db.Column(db.Integer, nullable=False, default=2, server_default='2')
    dictation_grade4_punctuation_max = db.Column(db.Integer, nullable=False, default=2, server_default='2')
    dictation_grade3_spelling_max = db.Column(db.Integer, nullable=False, default=4, server_default='4')
    dictation_grade3_punctuation_max = db.Column(db.Integer, nullable=False, default=4, server_default='4')
    dictation_use_grammar_errors = db.Column(db.Boolean, nullable=False, default=False, server_default=db.text('FALSE'))
    dictation_use_corrections = db.Column(db.Boolean, nullable=False, default=False, server_default=db.text('FALSE'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
    updated_by = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=True)
    manual_status = db.Column(db.String(30), nullable=True)
    ...
    logs = db.relationship("ControlWorkLog", backref="control_work", lazy=True, cascade="all, delete-orphan", order_by="desc(ControlWorkLog.created_at)")
    ...
    def work_kind_label(self):
        return self.WORK_KIND_LABELS.get(self.work_kind or self.WORK_KIND_CONTROL, "Контрольная работа")
...
class ControlWorkResult(db.Model):
    ...
    RESULT_STATUS_PRESENT = "present"
    RESULT_STATUS_ABSENT = "absent"
    RESULT_STATUS_CHOICES = [
        (RESULT_STATUS_PRESENT, "Писал"),
        (RESULT_STATUS_ABSENT, "Отсутствовал"),
    ]
    ...
    result_status = db.Column(db.String(20), nullable=False, default=RESULT_STATUS_PRESENT, server_default=RESULT_STATUS_PRESENT)
    is_absent = db.Column(db.Boolean, nullable=False, default=False, server_default=db.text('FALSE'))
    dictation_mark = db.Column(db.Integer, nullable=True)
    grammar_mark = db.Column(db.Integer, nullable=True)
    final_mark = db.Column(db.Integer, nullable=True)
    spelling_errors = db.Column(db.Integer, nullable=True)
    punctuation_errors = db.Column(db.Integer, nullable=True)
    grammar_errors = db.Column(db.Integer, nullable=True)
    corrections_count = db.Column(db.Integer, nullable=True)
...
class ControlWorkLog(db.Model):
    __tablename__ = "control_work_log"
...
class OlympiadResult(db.Model):
    ...
    percent = db.Column(db.Float, nullable=True)
    status = db.Column(db.String(500), nullable=True, index=True)
    ...
    teacher_binding_reason = db.Column(db.Text, nullable=True)
...
class OlympiadUnmatchedRow(db.Model):
    ...
    raw_status = db.Column(db.String(500), nullable=True)
...
class OlympiadSubjectMapping(db.Model):
    ...
    def linked_subject_id_list(self):
        values = []
        for raw in str(self.linked_subject_ids or "").replace(';', ',').split(','):
            raw = str(raw).strip()
            if raw.isdigit():
...
class KnowledgeArticle(db.Model):
    """База знаний: инструкции и материалы по ролям."""
    __tablename__ = "knowledge_article"

    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(255), nullable=False)
    body = db.Column(db.Text, nullable=True)          # текст статьи (Markdown или plain)
    link = db.Column(db.String(1024), nullable=True)   # внешняя ссылка
    file_path = db.Column(db.String(512), nullable=True)  # загруженный файл
    # тип: 'article' | 'link' | 'file'
    kind = db.Column(db.String(20), nullable=False, default="article")
    # JSON-список кодов ролей, которым доступна статья; [] = для всех
