...
PERMISSIONS = {
    ...
    "incident_dashboard_view": {
        ADMIN, METHODIST, SOCIAL_PEDAGOG, PSYCHOLOGIST
    },
...
def _user_role_codes(user=None) -> set:
    ...
    if user is None:
        try:
            from flask import g
            preview = getattr(g, "_hub_preview_role_codes", None)
...
def can_view_child_basic(child, user=None) -> bool:
    ...
    if has_role(DEPUTY_DIRECTOR, user=user):
        return True
    ...
    if has_role(CLASS_TEACHER, user=user):
        return is_class_teacher_of_child(child, user=user)
