...
def movements_registry():
    ...
    rows = ChildMovement.query.options(
        joinedload(ChildMovement.child),
        joinedload(ChildMovement.from_class),
        joinedload(ChildMovement.to_class),
        joinedload(ChildMovement.creator),
    )
    if year_id:
