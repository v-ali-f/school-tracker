    ...
    for row in rows:
        try:
            ...
            if existing:
                duplicate_rows += 1
                continue
            binding_status = "auto" if load and load.teacher_id else "unassigned"
            binding_source = "auto_load" if load and load.teacher_id else "unassigned"
            binding_reason = teacher_error or ("Учитель найден автоматически" if load and load.teacher_id else "Учитель не найден")
            result = OlympiadResult(
                ...
                teacher_binding_status=binding_status,
